...
`~/.cache/opencode`、`~/.local/share/opencode`、`~/.local/state/opencode`、
`~/.local/share/oh-my-opencode` 与 `~/.opencode/skills` 导入初始配置。
...
`.devcontainer/state/codex`、`.devcontainer/state/opencode-cache`、
`.devcontainer/state/opencode-config`、`.devcontainer/state/opencode-data`、
`.devcontainer/state/opencode-local-state`、`.devcontainer/state/oh-my-opencode-data`
或 `.devcontainer/state/opencode-skills` 目录（保留 `.gitignore` 即可），
再重新启动容器。
