...
public:
    DeformableInfantry()
        ...
        // 修正成员变量名：command_component_ -> deformable_infantry_command_
        ...
        // 初始化关节板
        joint_board_ = std::make_unique<JointBoard>(
            *this, *deformable_infantry_command_,
...
private:
    // 前置声明，以便 DeformableInfantry 使用 shared_ptr
    ...
    void steers_calibrate_subscription_callback(std_msgs::msg::Int32::UniquePtr) {
        if (chassis_board_) {
            RCLCPP_INFO(get_logger(), "[steer calibration] New left front offset: %ld",
                        chassis_board_->chassis_steer_motors_[0].calibrate_zero_point());
            RCLCPP_INFO(get_logger(), "[steer calibration] New left back offset: %ld",
                        chassis_board_->chassis_steer_motors_[1].calibrate_zero_point());
            RCLCPP_INFO(get_logger(), "[steer calibration] New right back offset: %ld",
                        chassis_board_->chassis_steer_motors_[2].calibrate_zero_point());
            RCLCPP_INFO(get_logger(), "[steer calibration] New right front offset: %ld",
    ...
    public:
        ...
        void command_update() {
            ...
            // 下面这行逻辑可能有误（发往 0x1FE？），我先保留你原有的逻辑
    ...
    class JointBoard final : private librmcs::client::CBoard {
    public:
        friend class DeformableInfantry;
    ...
    private:
        ...
        // 注意：JointBoard 需要声明自己的 buffer
        TransmitBuffer transmit_buffer_; 
        std::thread event_thread_;
    };

    std::shared_ptr<DeformableInfantryCommand> deformable_infantry_command_;
    std::shared_ptr<ChassisBoard> chassis_board_;
    std::shared_ptr<JointBoard> joint_board_;
