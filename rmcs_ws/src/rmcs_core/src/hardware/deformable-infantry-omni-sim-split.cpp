...
template <size_t N>
std::array<std::string, N> read_string_array_parameter(
    const rclcpp::Node& node, const char* name, const std::array<std::string, N>& defaults) {
    const auto values = node.get_parameter_or(name, std::vector<std::string>{defaults.begin(), defaults.end()});
    if (values.size() == N) {
...
double value_or_nan(const std::vector<double>& values, size_t index) {
    return index < values.size() ? values[index] : kNan;
}

template <size_t N>
std::optional<size_t> find_index(const std::string& name, const std::array<std::string, N>& names) {
    for (size_t i = 0; i < N; ++i) {
        if (name == names[i])
            return i;
    }
...
double read_control_input(const rmcs_executor::Component::InputInterface<double>& input) {
    return (input.ready() && std::isfinite(*input)) ? *input : 0.0;
}

} // namespace

class DeformableInfantryOmniSimState
    : public rmcs_executor::Component
    , public rclcpp::Node {
public:
    DeformableInfantryOmniSimState()
        : Node(
              get_component_name(),
              rclcpp::NodeOptions{}.automatically_declare_parameters_from_overrides(true))
        , joint_names_(read_string_array_parameter(
              *this,
              "joint_names",
              std::array<std::string, kJointCount>{
                  "left_front_joint", "left_back_joint", "right_back_joint", "right_front_joint"}))
        , wheel_names_(read_string_array_parameter(
              *this,
              "wheel_names",
              std::array<std::string, kJointCount>{
                  "left_front_wheel", "left_back_wheel", "right_back_wheel", "right_front_wheel"}))
        , joint_max_torque_(read_double_array_parameter(
              *this, "joint_max_torque", std::array<double, kJointCount>{25.0, 25.0, 25.0, 25.0}))
        , wheel_max_torque_(read_double_array_parameter(
              *this,
              "wheel_max_torque",
              std::array<double, kJointCount>{3.436926, 3.436926, 3.436926, 3.436926}))
        , joint_zero_physical_angle_rad_(
              deg_to_rad(get_parameter_or("joint_zero_physical_angle_deg", 62.5)))
        , control_power_limit_(get_parameter_or("control_power_limit", 500.0))
        , radius_base_(get_parameter_or("radius_base", 0.2341741))
        , radius_arm_length_(get_parameter_or("radius_arm_length", 0.150)) {
        const auto default_base_target_deg = read_double_array_parameter(
            *this,
            "base_target_physical_angle_deg",
            std::array<double, kJointCount>{58.0, 58.0, 58.0, 58.0});
    ...
    void update() override {
        ...
        *chassis_imu_roll_ = state.roll;
        ...
        for (size_t i = 0; i < kJointCount; ++i) {
            ...
            const double measured_torque = state.joints[i].effort;
            ...
            *joint_velocity_outputs_[i] =
                std::isfinite(physical_velocity) ? -physical_velocity : kNan;
        ...
        *chassis_radius_ = all_joint_angles_finite ? calculate_radius_(physical_angles) : radius_base_;
        ...
        for (size_t i = 0; i < kJointCount; ++i) {
            ...
            *wheel_torque_outputs_[i] = state.wheels[i].effort;
...
private:
    void register_joint_interfaces_() {
        for (size_t i = 0; i < kJointCount; ++i) {
            ...
            register_output("/chassis/" + name + "/angle", joint_angle_outputs_[i], kNan);
            register_output("/chassis/" + name + "/velocity", joint_velocity_outputs_[i], kNan);
            ...
            register_output("/chassis/" + name + "/physical_angle", joint_physical_angle_outputs_[i], kNan);
            register_output("/chassis/" + name + "/physical_velocity", joint_physical_velocity_outputs_[i], kNan);
            register_output(
    ...
    void register_wheel_interfaces_() {
        for (size_t i = 0; i < kJointCount; ++i) {
            ...
            register_output("/chassis/" + name + "/velocity", wheel_velocity_outputs_[i], kNan);
    ...
    void joint_state_subscription_callback(sensor_msgs::msg::JointState::UniquePtr msg) {
        ...
        for (size_t i = 0; i < msg->name.size(); ++i) {
            const double position = value_or_nan(msg->position, i);
            const double velocity = value_or_nan(msg->velocity, i);
            const double effort = value_or_nan(msg->effort, i);

            if (const auto joint_index = find_index(msg->name[i], joint_names_)) {
                state_.joints[*joint_index] = {position, velocity, effort};
                continue;
            }
            if (const auto wheel_index = find_index(msg->name[i], wheel_names_))
...
class DeformableInfantryOmniSimActuator
    : public rmcs_executor::Component
    , public rclcpp::Node {
public:
    DeformableInfantryOmniSimActuator()
        : Node(
              get_component_name(),
              rclcpp::NodeOptions{}.automatically_declare_parameters_from_overrides(true))
        , joint_names_(read_string_array_parameter(
        ...
        , wheel_names_(read_string_array_parameter(
              *this,
              ...
              std::array<std::string, kJointCount>{
                  "left_front_wheel", "left_back_wheel", "right_back_wheel", "right_front_wheel"}))
        , joint_command_topics_(read_string_array_parameter(
              *this,
        ...
        , wheel_command_topics_(read_string_array_parameter(
              *this,
        ...
        for (size_t i = 0; i < kJointCount; ++i) {
            register_input("/chassis/" + joint_names_[i] + "/control_torque", joint_control_torque_inputs_[i], false);
            register_input("/chassis/" + wheel_names_[i] + "/control_torque", wheel_control_torque_inputs_[i], false);
    ...
    void update() override {
        ...
        for (size_t i = 0; i < kJointCount; ++i) {
            msg.data = read_control_input(joint_control_torque_inputs_[i]);
            if (joint_command_publishers_[i])
                joint_command_publishers_[i]->publish(msg);

            msg.data = read_control_input(wheel_control_torque_inputs_[i]);
...
PLUGINLIB_EXPORT_CLASS(rmcs_core::hardware::DeformableInfantryOmniSimState, rmcs_executor::Component)
PLUGINLIB_EXPORT_CLASS(rmcs_core::hardware::DeformableInfantryOmniSimActuator, rmcs_executor::Component)
