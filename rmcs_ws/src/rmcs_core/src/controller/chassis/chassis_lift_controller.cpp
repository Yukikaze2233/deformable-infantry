...
public:
    ...
    {        
        ...
        register_input("/remote/joystick/left", remote_left_joystick_);
        register_input("/remote/joystick/right", remote_right_joystick_);
        ...
        register_output("/chassis/lift/lb/control_angle_error", lb_angle_error_, nan_);
        register_output("/chassis/lift/rf/control_angle_error", rf_angle_error_, nan_);
        register_output("/chassis/lift/rb/control_angle_error", rb_angle_error_, nan_);

        register_output("/target/input", target_input, nan_);
    ...
    void update() override {
        if ((*remote_left_switch_ == rmcs_msgs::Switch::DOWN || *remote_left_switch_ == rmcs_msgs::Switch::UNKNOWN)
            ...
            *left_front_wheel_torque_ = -1.0* remote_left_joystick_-> x();
            *left_back_wheel_torque_ = -1.0* remote_left_joystick_-> y();
            *right_front_wheel_torque_ = -1.0* remote_right_joystick_-> x();
            *right_back_wheel_torque_ = -2.0* remote_right_joystick_-> y();
            ...
            /*test 响应速度*/
            // s_target = trapezoidal_calculator(30.0 + 15 * sin(i * 0.005 * 0.25 * M_PI)/* *target_angle_ */);
            // *target_input = 319 - (30.0 + 15 * sin(i * 0.005 * 0.25 * M_PI)); 
            // i++;                    
            s_target = trapezoidal_calculator(*target_angle_);

            s_lf = trapezoidal_calculator( 319 - *left_front_wheel_angle_);
            s_lb = trapezoidal_calculator(211.9 - *left_back_wheel_angle_);
            if(*right_front_wheel_angle_ > 180){
            ...
            double lf_err = s_lf - s_target;
            double lb_err = s_lb - s_target;
            double rf_err = s_rf - s_target;
            double rb_err = s_rb - s_target;
            ...
            // auto now = std::chrono::steady_clock::now();     //当前时间戳
...
private:
    ...
    double trapezoidal_calculator(double alpha) const{
        ...
        auto s = term + sqrt(L * L - (Bx * std::sin(alpha * pi / 180) - By * std::cos(alpha * pi / 180) + 15) * (Bx * std::sin(alpha * pi / 180) - By * std::cos(alpha * pi / 180) + 15)) ;
        return s;
    }
    ...
    OutputInterface<double> target_input;

    OutputInterface<double> raw_angle;
};
